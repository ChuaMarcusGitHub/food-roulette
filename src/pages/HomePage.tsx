...
import { ROUTES, MIN_PASSWORD_LENGTH, INVITE_CODE_LENGTH } from "@/constants";
...
export default function HomePage() {
  ...
  const autoRedirect = useCallback(async () => {
    ...
    const { data: member } = await supabase
      ...
      .maybeSingle();
    if (member) navigate(ROUTES.GROUP(gid), { replace: true });
  }, [supabase, navigate]);
  ...
  async function handleCreateGroup(e: FormEvent) {
    ...
    try {
      ...
      const { data: group, error: gErr } = await createGroup(supabase, gName, inviteCode);
      ...
      const { data: memRow, error: mErr } = await supabase
        .from("group_members")
        .upsert({ group_id: group.id, display_name: you, device_id: deviceId }, { onConflict: "group_id,device_id" })
        .select("id")
        .single();
      ...
      const setPwErr = await setMemberPassword(supabase, group.id, deviceId, pw);
