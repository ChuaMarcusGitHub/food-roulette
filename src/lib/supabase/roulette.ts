...
import type { RouletteRun, NormalisedRun } from "@/types";
import { DEFAULT_TICK_MS } from "@/constants";

/** Normalise a raw roulette run row into the client shape. */
export function normaliseRun(row: Partial<RouletteRun> | null): NormalisedRun | null {
  if (!row) return null;
  return {
    id: row.id ?? "",
...
export function parseRpcRun(raw: unknown): NormalisedRun | null {
  if (!raw || typeof raw !== "object") return null;
  return normaliseRun(raw as Partial<RouletteRun>);
}
...
export async function fetchLatestRun(
  ...
  const { data } = await supabase
    ...
    .limit(1)
    .maybeSingle();
  return data ? normaliseRun(data as RouletteRun) : null;
}
...
export async function startRoulette(
  supabase: SupabaseClient,
  groupId: string,
): Promise<{ run: NormalisedRun | null; skipped: boolean; error: string | null }> {
  ...
  if ((data as Record<string, unknown>)?.skipped) return { run: null, skipped: true, error: null };
...
export async function voteReroll(
  ...
  runId: string,
  memberId: string,
): Promise<{ newRun: NormalisedRun | null; majority: boolean; error: string | null }> {
  ...
  let newRun: NormalisedRun | null = null;
